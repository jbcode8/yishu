...
class AuthGroupController extends AdminController{
    ...
    public function authorization(){
        if(IS_POST){
            ...
            $map['uid'] = $uid;
            $user_group_data = M('AuthGroupAccess')->where($map)->getField('group_id',true);
            $user_group = array();
            ...
            $groupList = M('AuthGroup')->field('id,title')->where(array('module'=>'admin'))->select();
    ...
    public function groupmember($group_id){
        $group = M('AuthGroup')->field('title')->find($group_id);

        $admin_table  = C('DB_PREFIX').'admin';
        $group_table  = C('DB_PREFIX').'auth_group_access';
    ...
    public function removeauth($uid,$group_id){
        if(M('AuthGroupAccess')->where(array('uid'=>$uid,'group_id'=>$group_id))->delete()){
            $this->success('解除授权成功!');
        }else{
            $this->error('解除授权失败!');
        }
    ...
    public function access(){
        if(IS_POST){
            ...
            if(M('AuthGroup')->save($data) !== false){
                $this->success('授权成功!');
            }else{
                $this->error('授权失败!');
            }
        }else{
            $rule = M('AuthRule')->field('id,pid as pId,title as name')->where(array('module'=>'admin','status'=>1))->order('listorder asc,id asc')->select();
            $groupRule = M('AuthGroup')->field('rules')->find((int)$_REQUEST['id']);
