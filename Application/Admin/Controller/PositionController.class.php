...
class PositionController extends AdminController {
    ...
    public function delete($posid = null){
        $res = M('Position')->delete($posid);
        if($res){
            M('PositionData')->where(array('posid'=>$posid))->delete();
            $this->success('删除成功!');
        }else{
            $this->error('删除失败!');
        }
    ...
    public function items($posid){
        $data = D('PositionData')->where(array('posid'=>$posid))->select();
    ...
    public function delete_data($id = null,$model_id = null,$posid = null){
        ...
        if(M('PositionData')->where($map)->delete()){
            $this->success('删除成功!');
        }else{
            $this->error('删除失败!');
        }
