...
class MbrandController extends MpublicController {
    ...
    public function _initialize(){
        ...
        $this->Model = D('Brand');
        $this->aryAct = array('add', 'edit', 'status', 'list');
        $this->where = array('store_id' => $_SESSION['store_id']);
        $this->indexId = 'brand_id';
    }
    ...
    private function _list($where){
        ...
        // 查询
        $aryList = $this->Model->field(array('brand_id','brand_name','status','create_time','brand_logo','brand_desc'))->where($where)->select();
