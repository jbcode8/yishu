...
class AddonsController extends AdminController {
    ...
    public function create(){
        ...
        $hooks = M('HooksMall')->field('name,description')->select();
...
str;
    ...
    public function saveConfig(){
        $id     =   (int)I('id');
        $config =   I('config');
        $flag = M('AddonsMall')->where("id={$id}")->setField('config',json_encode($config));
        if($flag !== false){
    ...
    public function install(){
        ...
        if($addonsModel->add($data)){
            ...
            $hooks_update   =   D('HooksMall')->updateHooks($addon_name);
            if($hooks_update){
                S('hooks', null);
                $this->success('安装成功');
            }else{
    ...
    public function uninstall(){
        ...
        $hooks_update = D('HooksMall')->removeHooks($db_addons['name']);
    ...
    public function hooks(){
        $list   =   parent::lists('Hooks');
        int_to_string($list, array('type'=>array('1'=>'视图','2'=>'控制器')));
    ...
    public function edithook($id){
        $hook = M('HooksMall')->field(true)->find($id);
    ...
    public function delhook($id){
        if(M('HooksMall')->delete($id) !== false){
            $this->success('删除成功');
        }else{
            $this->error('删除失败');
        }
