...
class DocumentModel extends Model{
    ...
    public function position($id,$data){
        ...
        foreach($posid as $k=>$v){
            ...
            $array[$v]['posid'] = $v;
            $array[$v]['is_thumb'] = $data['thumb']?1:0;
        ...
        if(!empty($data['id'])){
            ...
            //获取已存在的推荐位
            $unpos = $pos = $posDataModel->where(array('id'=>$id,'model_id'=>$data['model']))->getField('posid',true);
            foreach($pos as $k=>$v){
                foreach($posid as $pk=>$p){
                    if($v == $p){
                        unset($pos[$k],$posid[$pk]);
                    }
                }
            }
