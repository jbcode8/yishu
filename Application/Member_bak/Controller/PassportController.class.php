...
class PassportController extends HomeController{
    ...
    public function login(){
        if(IS_POST){
            ...
            $User = new UserApi();
            $uid = $User->login($username,$password,login_type($username));
            if(0 < $uid){ //登录成功
                /* 登录用户 */
                $Member = D('Member');
                if($Member->login($uid,$remember)){ //登录用户
                    $returnUrl = $_GET['returnUrl']?$_GET['returnUrl']:U(__APP__."/Member");
                    redirect($returnUrl);
                } else {
    ...
    public function logout(){
        D('Member')->logout();
        $returnUrl = $_GET['returnUrl']?$_GET['returnUrl']:U(__APP__."/Member/Passport/login");
        redirect($returnUrl);
    }
