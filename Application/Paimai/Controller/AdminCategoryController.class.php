...
class AdminCategoryController extends AdminController{
    ...
    public function index(){
       /* $sql="select a.attr_name from yishu_paimai_attribute a where a.attr_id in (select cat_attrid from yishu_paimai_category c where c.cat_id=4)";
       M()->query($sql);
       p(M()->query($sql));*/
        $field=array(
            "*",
...
#插入数据
    public function add(){
    	$this->top_cat_list=M("PaimaiCategory")->where("cat_pid=0")->select();
    	//p($top_cat_list);
        //调用控制器AdminAttributeController下面的一个方法获得商品类型列表
        $this->goodstype=A("AdminAttribute")->getGoodsType();
        $this->display();
    }
    public function insert(){
		//p(array_unique($_POST['attr_id']));
        $cat=D("PaimaiCategory");
        if($cat->create()){
            //得到铡插入的分类id
            $cat_id=$cat->add($_POST);
     #下面代码要进行优化
            $cat_attr=M("PaimaiCatAttr");
            //业务逻辑:1去除重复的可筛选属性,2去除0的可筛选属性
            $attrArr=array_unique($_POST['attr_id']);
            foreach($attrArr as $k=>$v){
                //如果本属性为0,则删除,如果不为0则插入yishu_paimai_cat_attr表
                if($v==0){
                    unset($attrArr[$k]);
                }else{
                    $data['cat_attr_catid']=$cat_id;
                    $data['cat_attr_attrid']=$v;
                    $cat_attr->data($data)->add();
                }
            }
...
#编辑数据
    public function edit(){
        ...
        $this->cat=M("PaimaiCategory")->find($cat_id);
		
        //得到商品类型列表
        $this->goodstype=A("AdminAttribute")->getGoodsType();
        //得到本分类下的可筛选属性
        $this->cat_attr=M("PaimaiCatAttr")->where("cat_attr_catid=".$cat_id)->select();
    ...
    public function update(){
        ...
        if($cat->create()){
            ...
            $cat_attr=M("PaimaiCatAttr");
			//先删除分类属性对照表中的数据
            $cat_attr->where("cat_attr_catid=$cat_id")->delete();
            //业务逻辑:1去除重复的可筛选属性,2去除0的可筛选属性
            $attrArr=array_unique($_POST['attr_id']);
            $tag=true;
            foreach($attrArr as $k=>$v){
...
#删除数据
    public function delete(){
        ...
        //$tag=true;
        //更新商品分类
        //如果删除本分类则把本分类下的所有商品的分类更为0,0为[其它分类]
        $good_del=M("PaimaiGoods")->where("goods_catid=$cat_id")->setField(array("goods_catid"=>0));
        //写入删除字段,同时把isshow改变0
        $cat_del=M("PaimaiCategory")->where("cat_id=".$cat_id)->setField(array("cat_isdelete"=>1,"cat_isshow"=>0));
        if($cat_del){
            $this->success("删除成功");
        }else{
            $this->error("删除失败");
...
#ajax区域
    /*
     * 当添加商品分类的时候ajax请求某类型下的属性
     * 传入goodstype_id
     * 返回attributy中的属性
     */
    public function ajax_getattribute(){
        if(!IS_AJAX) $this->error("你请求的页面不存在");
        $attribute=M('PaimaiAttribute')->where("attr_goodstypeid=".$_GET['goodstype_id'])->select();

        //$attribute['status']=1;
        $this->ajaxReturn($attribute,"json");
        //echo json_encode($attribute);

    }
    /*
     * 当编辑商品分类时候ajax请求该分类下的可筛选属性
     * 传入attr_id从yishu_paimai_cat_attr中查找
     * 返回可筛选属性的信息
     */
    public function ajax_get_attr_info(){
		
        if(!IS_AJAX) $this->error("你请求的页面不存在");
        $sql="select yishu_paimai_attribute.attr_id,yishu_paimai_attribute.attr_name,yishu_paimai_attribute.attr_goodstypeid from yishu_paimai_attribute where attr_goodstypeid=(select attr_goodstypeid from yishu_paimai_attribute where attr_id=".$_GET['cat_attr_attrid'].")";
