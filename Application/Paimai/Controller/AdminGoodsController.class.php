...
class AdminGoodsController extends AdminController
{
    ...
    #接受参数
		...
		//征集人筛选
        $recruiter_id=I('recruiter',0,'intval');
        if(!empty($recruiter_id)){
            $where.="goods_recruiterid=$recruiter_id and ";
        }
        //征集人列表
        $this->recruiter = M('PaimaiRecruiter')->field("recruiter_id,recruiter_name")->where("recruiter_isshow=0")->order("recruiter_id asc")->select();
		//p($this->recruiter);
        $recruiter_str="";
        foreach($this->recruiter as $k=>&$v){
            if($recruiter_id==$v['recruiter_id']){
                $recruiter_str.="<option selected='selected' value=".$v['recruiter_id'].">".$v['recruiter_name']."</option>";
            }else{
                $recruiter_str.="<option value=".$v['recruiter_id'].">".$v['recruiter_name']."</option>";
            }
        }
        $this->recruiter_str=$recruiter_str;
		
		//商家筛选
        $seller_id=I('seller',0,'intval');
		//p($seller_id);
        if(!empty($seller_id)){
            $where.="goods_sellerid=$seller_id and ";
        }
		//p($where);
        //商家列表
		...
		//p($this->seller);
        $seller_str="";
        foreach($this->seller as $k=>&$v){
            if($seller_id==$v['seller_id']){
                $seller_str.="<option selected='selected' value=".$v['seller_id'].">".$v['seller_name']."</option>";
            }else{
                $seller_str.="<option value=".$v['seller_id'].">".$v['seller_name']."</option>";
            }
        }
        $this->seller_str=$seller_str;
	...
	#下载，
		...
		//如果是下载则走if里面的
		if($down=='down'){
			$downgoods_field=array(
				'goods_id'=>"商品ID",
				'goods_name'=>"商品名称",
				'goods_sn'=>"商品编号",
				'goods_specialid'=>"专场ID",
				'(select recruiter_name from yishu_paimai_recruiter where recruiter_id=goods_recruiterid)'=>"征集人员",
				'(select seller_name from yishu_paimai_seller where seller_id=goods_sellerid)'=>"所属商家",
				"from_unixtime(goods_starttime,'%Y-%m-%d')"=>"开始时间",
				"from_unixtime(goods_endtime,'%Y-%m-%d')"=>"结束时间",
				'goods_cost'=>"保留价",
            );
			$downgoods_arr = M("PaimaiGoods")->field($downgoods_field)->where($where)->order("goods_id desc")->select();
			//第一个数组为二维数组数据,第二个数组$field为一维数组title
			foreach($downgoods_arr as $k=>&$v){
				$v['URL']="http://www.yishu.com/paimai/goods-".$v['商品ID'].".html";
			}
			$arr=array(
				'data'=>$downgoods_arr,
				'title'=>$downgoods_field,
			);
			Vendor('Csv.Csv');
			$CSV = new \Csv;
			$CSV->export_csv($arr);
        ...
        $p=($p-1)*$prePage;
		//分页商品
		//$this->lists = M("PaimaiGoods")->field($field)->where($where)->page(11 . ',' . 20)->order("goods_id desc")->select();
        $this->lists = M("PaimaiGoods")->field($field)->where($where)->limit("{$p},20")->order("goods_id desc")->select();
        //echo M("PaimaiGoods")->getLastSql()."<br/>";
		//p($this->lists);
		//分页商品总数
    ...
    public function add()
    {
       ...
       // p($this->admin);
		...
		$this->seller = M('PaimaiSeller')->field("seller_id,seller_name")->where("seller_isshow=0")->order("seller_id asc")->select();
		//征集人员
		$this->recruiter = M('PaimaiRecruiter')->field("recruiter_id,recruiter_name")->where("recruiter_isshow=0")->order("recruiter_id desc")->select();
        //拍卖专场
	   ...
	   $admin_where=array(
        ...
        if ($goods->create()) {
			...
			if( $admin[0]['admintype']==1){
                   $goods_isshow=0;
                   $goods->goods_isshow = $goods_isshow;

			}
			//p($goods->goods_isshow);
            //添加后返回刚生成的商品id
