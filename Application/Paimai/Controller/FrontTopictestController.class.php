...
class FrontTopictestController extends PaimaiPublicController
{
    ...
    public function index()
    {
		...
		//不要分页
		$list = M('PaimaiGoods')->field($list_field)->where($where2)->order($order)->limit(15)->select();
		//不要分页结束
        foreach ($list as $k => $v) {
    ...
    public function ajax_loadmore(){
        ...
        if(empty($goods_arr)){
            ...
            $this->ajaxReturn(array('status'=>0, 'msg' => '请求的栏目不存在'), 'json');
            exit;
        }
