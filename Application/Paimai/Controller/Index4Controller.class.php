...
class Index4Controller extends PaimaiPublicController
{
	...
	#专场焦点图 #今日10:00场
		...
		// if(date('H') < F('time')['settime'] && date('H') >=0 ){
		// 	$special_where=
		// 		"special_isshow=1 and special_isdelete=0 and from_unixtime(special_starttime,'%Y-%m-%d')='".date('Y-m-d',strtotime('-1 day'))."'";
		// }else{
		// 	$special_where=
		// 		"special_isshow=1 and special_isdelete=0 and from_unixtime(special_starttime,'%Y-%m-%d')='".date('Y-m-d')."'";
		// }
		

		//晚上十点以后到0点查询条件
		if(date('H') > 22){//如果当前时间大于晚十点
			$special_where = array(
			'special_isshow' => 1,
			'special_isdelete' => 0,
			'special_starttime' => array('between',array(strtotime(date("Y-m-d", time()))+22*3600-1, strtotime(date("Y-m-d", time()))+24*3600))
			);
		} else {
			$special_where = array(
			'special_isshow' => 1,
			'special_isdelete' => 0,
			'special_starttime' => array('between',array(strtotime(date("Y-m-d", time()))-2*3600-1, strtotime(date("Y-m-d", time()))+22*3600-1))
			);
		}
		//修改为四个图片
		$Special=M("PaimaiSpecial")->field($special_field)->where($special_where)->order("special_id desc")->limit(10)->select();
		
		//如果为空，调用最后四个专场
		if(empty($Special)){
			$Special=M("PaimaiSpecial")->where("special_isshow=1 and special_isdelete=0")->order("special_id desc")->limit(10)->select();
		}
	...
	#今日22:00场
		// echo date("Y-m-d H:i:s", strtotime(date("Y-m-d", time()))+22*3600-1);
		// echo '<br />';
		// echo date("Y-m-d H:i:s", strtotime(date("Y-m-d", time()))+46*3600-1);
		$special_field = 'special_id, special_name, special_starttime, special_endtime, recordid';
		if(date('H') >22){//如果当前时间大于晚十点
			$special_where = array(
				'special_isshow' => 1,
		...
		//修改为四个图片
		$Speciale=M("PaimaiSpecial")->field($special_field)->where($special_where)->order("special_id desc")->limit(10)->select();
		...
		//如果为空，调用最后四个专场
		// if(empty($Speciale)){
		// 	$Speciale=M("PaimaiSpecial")->where("special_isshow=1 and special_isdelete=0")->order("special_id desc")->limit(5)->select();
		// }
		...
		// 	// //出价次数
		// 	// $Speciale[$k]['bidtimes'] = M('PaimaiGoods')->where(array('goods_specialid'=>$v['special_id']))->sum('goods_bidtimes');
		//  }
		 //print_r($Speciale);
  ...
  //           $IngGoods[$k]['thumb']=str_replace("..",'',img($v['index_img']));
	...
	#热门拍品
		...
		$HotGoods=M("PaimaiGoods")->field($hotgoods_field)->where($hotgoods_where)->limit(4)->order("goods_order desc,goods_nowprice asc,goods_id asc")->/*page($p . ',' . $prePage)->*/select();
