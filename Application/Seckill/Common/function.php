...
function getskspecial($skspecial_id){
    $skspecial=M('SeckillSpecial')->find($skspecial_id);
    return $skspecial['skspecial_name'];
}
/*规则end*/
/*
    些函数为获得用户信息
...
//传入yishu_seckill_order_info中的id返回这个订单下面的商品
function getOrdergoodsByOrderinfo($orderinfo_id)
{
    $order = M("SeckillOrder")->where('order_orderinfoid=' . $orderinfo_id)->select();
    $str = "";
    foreach ($order as $k => $v) {
        $str .= "<tr>";
        $str .= "<td>" . $v['order_goodssn'] . "</td>";
        $str .= "<td><a href='/seckill/goods-".$v['order_goodsid'].".html'>" . $v['order_goodsname'] . "</a></td>";
        $str .= "<td>" . $v['order_number'] . "</td>";
        $str .= "<td>" . $v['order_goodskillprice'] . "</td>";
        $str .= "<td></td>";
        $str .= "</tr>";
    }
